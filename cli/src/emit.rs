...
fn render_inputs(inputs: &BTreeMap<String, InputTypeDef>) -> String {
    ...
    s.push_str("import Cachebay\n\n");
    for (_, t) in inputs {
        // A nullable input field is plain `Optional` (whose `nil` OMITS the key)
        // by DEFAULT; it becomes the tri-state `GraphQLNullable<T>` only when the
        // config marks it via `explicitNullable`. `@oneOf` members are never
        // tri-state — explicit null is invalid there, so the mark can't promote
        // them (`nil` already means omit).
        let field_tristate = |f: &InputField| !t.is_one_of && f.explicit_nullable;
        ...
        for f in &t.fields {
            s.push_str(&format!(
                ...
                crate::plan::swift_input_type(&f.shape, field_tristate(f))
            ));
        }
        // init
        ...
        let args: Vec<String> = t
            ...
            .iter()
            .map(|f| {
                let ty = crate::plan::swift_input_type(&f.shape, field_tristate(f));
        ...
        for f in &t.fields {
            let ident = swift_identifier(&f.name);
            s.push_str(&emit_input_field("        ", "out", &f.name, &ident, &f.shape, field_tristate(f)));
        }
...
fn render_typed_plan_impl(plan: &Plan, ctx: &EmitCtx) -> String {
    ...
    if !plan.variables.is_empty() {
        ...
        for v in &plan.variables {
            let ident = swift_identifier(&v.name);
            s.push_str(&emit_input_field("            ", "out", &v.name, &ident, &v.shape, v.tristate));
        }
...
mod codegen_tests {
    ...
    fn ifield(name: &str, shape: TypeShape) -> InputField {
        InputField { name: name.into(), graphql_type: String::new(), shape, explicit_nullable: false }
    }
    fn ifield_explicit(name: &str, shape: TypeShape) -> InputField {
        InputField { name: name.into(), graphql_type: String::new(), shape, explicit_nullable: true }
    }
    ...
    /// DEFAULT: an unmarked schema-nullable input field is a plain `Optional`
    /// whose `nil` OMITS the key (the real bug fix) — NOT the tri-state wrapper,
    /// and never the old `?? .null` that forced explicit null.
    #[test]
    fn unmarked_nullable_input_field_is_optional_and_omits() {
        let out = render_inputs(&itype(
            "UpdateInput",
            false,
            vec![ifield("name", ishape("String", TypeKind::Scalar, true, false, false))],
        ));
        assert!(out.contains("public var name: String?\n"), "type: {out}");
        assert!(out.contains("name: String? = nil"), "default: {out}");
        assert!(out.contains("if let name = name { out[\"name\"] = .string(name) }"), "encoder: {out}");
        assert!(!out.contains("GraphQLNullable"), "unmarked must NOT be tristate: {out}");
        assert!(!out.contains("?? .null"), "must not force explicit null: {out}");
    }

    /// OPT-IN: a field marked in `explicitNullable` becomes the tri-state wrapper
    /// and encodes via `__cachebayEncode` (`.none` omits, `.null` sends null).
    #[test]
    fn marked_nullable_input_field_is_tristate() {
        let out = render_inputs(&itype(
            "UpdateInput",
            false,
            vec![ifield_explicit("name", ishape("String", TypeKind::Scalar, true, false, false))],
        ));
    ...
    /// Non-null input fields are bare and always emit — the mark is irrelevant.
    #[test]
    ...
    /// DEFAULT list: an unmarked nullable list is `[String]?` + omit-on-nil.
    #[test]
    fn unmarked_nullable_list_is_optional_and_omits() {
        let out = render_inputs(&itype(
            "FilterInput",
            false,
            vec![ifield("tags", ishape("String", TypeKind::Scalar, true, true, false))],
        ));
        assert!(out.contains("public var tags: [String]?"), "type: {out}");
        assert!(
            out.contains("if let tags = tags { out[\"tags\"] = Cachebay.JSONValue.array(tags.map { .string($0) }) }"),
            "encoder: {out}"
        );
        assert!(!out.contains("GraphQLNullable"), "{out}");
    }

    /// OPT-IN list: a marked nullable list `[String!]` wraps the list as a whole.
    #[test]
    fn marked_nullable_list_is_tristate() {
        let out = render_inputs(&itype(
            "FilterInput",
            false,
            vec![ifield_explicit("tags", ishape("String", TypeKind::Scalar, true, true, false))],
        ));
    ...
    /// `@oneOf` members keep plain `Optional` EVEN when marked — explicit null is
    /// invalid for `@oneOf`, so the mark must not promote them to tri-state.
    #[test]
    fn oneof_input_field_stays_optional_even_when_marked() {
        let out = render_inputs(&itype(
            "SearchBy",
            true,
            vec![ifield_explicit("email", ishape("String", TypeKind::Scalar, true, false, false))],
        ));
