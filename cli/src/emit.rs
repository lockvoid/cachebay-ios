...
pub fn write_all(
    ...
    // The namespace enum is declared once (here); each typed operation/fragment
    // file extends it. Only meaningful for typed emission.
    let schema_namespace = if typed { namespace } else { "" };
    fs::write(
        out_dir.join("Schema.graphql.swift"),
        render_schema(interfaces, schema_namespace),
    )?;
    for plan in plans {
        let file_name = format!("{}.graphql.swift", plan.name);
        let contents = if typed {
            wrap_in_namespace(render_typed_plan(plan), namespace)
        } else {
