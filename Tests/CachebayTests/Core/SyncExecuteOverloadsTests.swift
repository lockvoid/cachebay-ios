...
final class SyncExecuteOverloadsTests: XCTestCase {
    ...
    private func seedPostQ(_ client: CachebayClient, title: String) throws {
        try client.write(
            query: PostQ.self, variables: .init(id: "p1"),
            data: PostQData(_dataDict: [
    ...
    func test_executeMutation_sync_returnsToken_andFiresOnData() {
        ...
        let token = client.execute(
            mutation: UpdatePostM.self, variables: .init(id: "p1", title: "renamed"),
            onData: { data in
    ...
    func test_executeMutation_sync_firesOnError_onNetworkError() {
        ...
        _ = client.execute(
            ...
            onError: { err in
                XCTAssertNotNil(err.networkError); exp.fulfill()
            })
        wait(for: [exp], timeout: 2.0)
    }
    ...
    func test_executeQuery_sync_firesOnCacheData_andOnNetworkData() throws {
        ...
        _ = client.execute(
            PostQ.self, variables: .init(id: "p1"), cachePolicy: .cacheAndNetwork,
            ...
            onNetworkData: { data in
                XCTAssertEqual(data.post?.title, "from network"); netExp.fulfill()
            },
            onError: { err in XCTFail("unexpected onError: \(err)") })
    ...
    func test_executeQuery_sync_cancel_suppressesNetworkCallback() {
        ...
        let token = client.execute(
            PostQ.self, variables: .init(id: "p1"), cachePolicy: .networkOnly,
    ...
    func test_executeSubscription_sync_firesOnData_perFrame() {
        ...
        let token = client.executeSubscription(
            PostUpdatedSub.self, variables: .init(id: "p1"),
            onData: { data in
                if let t = data.postUpdated?.title { collector.append(t) }; exp.fulfill()
            },
            onError: { err in XCTFail("unexpected onError: \(err)") })
        wait(for: [exp], timeout: 2.0)
    ...
    func test_executeQuery_sync_cacheFirstHit_firesOnCacheDataSync_beforeReturn() throws {
        ...
        order.append(1)
        _ = client.execute(
            PostQ.self, variables: .init(id: "p1"), cachePolicy: .cacheFirst,
    ...
    func test_executeQuery_sync_cacheFirstHit_firesOnCacheData_onCallingThread() throws {
        ...
        XCTAssertEqual(
            callbackThread.snapshot(), [ObjectIdentifier(callerThread)],
