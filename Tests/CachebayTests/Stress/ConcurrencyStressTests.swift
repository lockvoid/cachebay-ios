...
final class ConcurrencyStressTests: XCTestCase {
    ...
    func test_50_parallel_mutations_all_visible() async throws {
        ...
        for i in 0..<50 {
            XCTAssertEqual(client.graph.getField("Post:p\(i)", "title")?.string, "t-p\(i)",
    ...
    func test_parallel_optimistic_patches_and_reverts_no_deadlock() async throws {
        ...
        try client.writeFragment(id: "Post:p1", fragment: "fragment P on Post { id counter }", data: .object([
        ...
        XCTAssertEqual(client.graph.getField("Post:p1", "counter")?.int, 0,
    ...
    func test_watcher_never_drops_final_state_under_burst() async throws {
        ...
        try client.writeQuery(query: "query { post(id: \"p1\") { id title } }", data: .object([
        ...
        XCTAssertGreaterThanOrEqual(emissions.value.count, 2,
    ...
    func test_no_lock_ordering_deadlock_mixed_load() async throws {
        ...
        http.whenQueryContains("createPost") { vars in
            let id = vars["id"]?.string ?? "?"
            return OperationResult(data: .object([
                "createPost": .object(["post": .object([
                    "__typename": "Post", "id": .string(id), "title": .string("t-\(id)")
                ])])
            ]))
        }
        let client = makeClient(http: http)
        try client.writeFragment(id: "Post:base", fragment: "fragment P on Post { id title }", data: .object([
