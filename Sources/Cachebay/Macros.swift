...
//
// Implementations live in the `CachebayMacrosImpl` compiler plugin. Generated
...
public macro CachebayData(typename: String) =
    #externalMacro(module: "CachebayMacrosImpl", type: "CachebayDataMacro")

/// Per-field construction default, read by `@CachebayData`. A no-op marker on its own.
@attached(peer)
public macro CachebayDefault<T>(_ value: T) =
    #externalMacro(module: "CachebayMacrosImpl", type: "CachebayDefaultMacro")
...
public macro CachebayInterface() =
    #externalMacro(module: "CachebayMacrosImpl", type: "CachebayInterfaceMacro")

/// Like `@CachebayInterface`, but for unions: the only lifted shared field is `__typename`.
@attached(member, names: arbitrary)
public macro CachebayUnion() =
    #externalMacro(module: "CachebayMacrosImpl", type: "CachebayUnionMacro")
